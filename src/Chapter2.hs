...
firstHalf :: [a] -> [a]
firstHalf l = take midpoint l
    where
        midpoint = (length l) `div` 2
...
takeEven :: [a] -> [a]
takeEven [] = []
takeEven (x1:x2:xs) = x1 : takeEven xs
takeEven (x1:x2) = x1 : takeEven x2
...
contains value lists = filter (elem value) lists
...
rotate value xs
    | value < 0 = []
    | otherwise = take len (drop value (cycle xs))
    where
        len = length xs
